...
var C = {
    ...
    GOLD:'#F4D03F', SKY:'#5DADE2', CORAL:'#E74C3C', MINT:'#27AE60',
};
...
function infoTable(rows) {
    return new Table({width:{size:100,type:WidthType.PERCENTAGE},rows:rows.map(function(kv){return new TableRow({children:[
        new TableCell({width:{size:22,type:WidthType.PERCENTAGE},shading:{fill:C.LIGHT},children:[new Paragraph({children:[new TextRun({text:kv[0],size:20,font:'微软雅黑',bold:true,color:C.MAIN})],alignment:AlignmentType.RIGHT,spacing:{before:30,after:30}})]}),
        new TableCell({width:{size:78,type:WidthType.PERCENTAGE},children:[new Paragraph({children:[new TextRun({text:kv[1],size:20,font:'微软雅黑'})],spacing:{before:30,after:30}})]}),
    ]})})});
}
...
function redline(text) {
    return new Paragraph({children:[new TextRun({text:'⛔ '+text,size:21,font:'微软雅黑',bold:true,color:C.RED})],spacing:{after:80,line:360},indent:{left:300},border:{left:{style:BorderStyle.SINGLE,size:6,color:C.RED,space:8}}});
}
function greenCheck(text) {
    return new Paragraph({children:[new TextRun({text:'✅ '+text,size:21,font:'微软雅黑',color:C.GREEN})],spacing:{after:60,line:340},indent:{left:300}});
}
function warn(text) {
...
function buildDocument() {
    ...
    children.push(flowBox('链商2.0 · 链生活 — 面向社区商业的数字经营平台。商户独立经营 · 生态会员互通 · 消费权益流转 · 真实交易激励。交易即分润，零资金池，100%去金融中心化。', false));

    children.push(h2('1.2  五大投资亮点'));
    children.push(calloutBox('投资亮点', [
        '① 蓝海市场：中国社区商业市场规模¥1.3万亿，数字化渗透率<5%，增长空间巨大',
        '② 差异化定位：唯一"跨店通兑+交易即分润+零固定费用"的社区商业平台——不与美团/拼多多正面竞争',
        '③ 轻资产模式：无库存、无物流、无资金池——平台仅传递分账指令，汇付持牌机构直清',
        '④ 合规壁垒：三条红线全面合规+跨店通兑不构成二清+三级推广体系不构成传销——监管友好型商业模式',
        '⑤ 网络效应：商家越多→消费者越多→交易越多→分润越多→推广者越多→商家越多——六方增长飞轮',
    ]));
    ...
    children.push(n(4,'支付基建完善：汇付天下等持牌机构提供合规分账能力——使"交易即分润"在不碰资金的情况下技术上可行'));
    ...
    children.push(p('盈亏平衡点计算：¥190,000 ÷ ¥4.686/笔 ≈ 40,500笔/月。数据来源：V3.2分润核销模型。',{bold:true}));
    children.push(pageBreak());
    ...
    children.push(dataTable(
        ...
        [
            ...
            ['行业内竞争','中','目前无直接竞品（跨店通兑+交易分润+社区垂直）\n——但美团/抖音随时可能进入'],
        ]
    ));
    children.push(pageBreak());
    ...
    children.push(riskMatrix([
        ...
        ['二清/传销认定风险','低','极高','P0','代金券同业态·≤3层分润·零入门费·法律顾问定期审查'],
    ...
    children.push(greenCheck('三级推广体系≤3层分润·零入门费·收入基于交易量(非人头费)'));
