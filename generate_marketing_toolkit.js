...
// 赋商通（链商2.0）市场拓展营销工具包 V1.0
//
// 覆盖全层级工具：地推人员 → 市场部/BD → 招商会专用
// 5大部分 · 21个章节 · 单文件输出
//
// 输出：20260615 市场拓展营销工具包/赋商通_市场拓展营销工具包_V1.0.docx
// =============================================================================
...
function buildE1_ParameterReference() {
    var children = [].concat(
        h1('E1  全参数速查手册'),
        p('以下为赋商通平台全部核心参数，按类别整理，供各角色随时查阅。所有参数以 lib/constants.js 为准，修改须经运营提案→品牌评估→技术确认→法务审核→管理层审批。'),
        divider(),
...
function buildE2_BrandVisualSummary() {
    var children = [].concat(
        h1('E2  品牌视觉规范摘要'),
        p('以下为赋商通（链生活）品牌视觉识别系统的核心规范摘要。完整品牌执行手册见《链商小程序品牌执行手册》。'),
        divider(),
...
function buildA1_ElevatorPitch() {
    var children = [].concat(
        ...
        calloutBox('消费者30秒', [
            ...
            '【价值】赋商通是我们社区的商家经营平台——在附近任意合作商家消费，都能获得代金券、积分和消费金。更重要的是，在A店获得的券，在B店也能用！全平台通用，没有限制。',
            '【行动】扫码注册，新人即送¥25消费礼包。来，我帮你扫一下——'
        ], C.LIGHT),

        h3('🏪 面向商家（10+15+5秒）'),
        calloutBox('商家30秒', [
            '【吸引】老板您好！我是赋商通的服务人员——想跟您聊一个帮您提升复购率、且不收固定费用的方案。',
        ...
        calloutBox('推广者30秒', [
            ...
            '【价值】推广者在赋商通帮助商家入驻和消费者注册，每一笔真实交易都有佣金。平台商家的佣金池是9%，推广者分65%——不设入门费、零保证金、完全基于真实交易。',
        ...
        calloutBox('投资人/合作伙伴30秒', [
            ...
            '【价值】赋商通通过"交易即服务费结算+全平台权益互通+千面千店"三大差异化机制，构建了独特的竞争壁垒。盈亏平衡仅需40,500笔/月，轻资产、无库存、合规底线全部满足。',
        ...
        infoTable([
            ...
            ['四大核心机制', '① 商户独立经营（千面千店） ② 生态会员互通（全平台会员） ③ 消费权益流转（跨店通兑） ④ 真实交易激励（交易即服务费结算）'],
        ...
        dataTable(
            ...
            [
                ...
                ['2', '差异化定位', '唯一实现"跨店通兑+交易即服务费结算+零固定费用"的平台'],
    ...
    var children = [].concat(
        ...
        dataTable(
            ...
            [
                ...
                ['多久能见收入？', '商家上线后产生第一笔交易即开始有收入', '交易即服务费结算'],
...
function buildB1_DoorToDoorScripts() {
    var children = [].concat(
        ...
        scriptDialog([
            ['1', '推广者', '老板您好！我是赋商通在本社区的服务人员，今天专门来拜访咱们这片区的商家——方便聊2分钟吗？', '选老板空闲时段（非饭点/非忙时），不要上来就推销'],
            ['2', '推广者', '我先简单介绍一下：赋商通是咱们社区的数字经营平台，帮助商家提升复购率的。（递上平台一页纸）跟美团最大区别是——我们不收固定费用，只在您实际成交时收服务费。', '先用一句话引起好奇，再递资料；不要一次说太多'],
        ...
        scriptDialog([
            ['1', '推广者', '您好！咱们社区刚上线了一个新的消费平台——赋商通。以后在附近合作商家消费都有优惠。（递上单页）可以了解一下，扫个码就行！', '微笑+单手递单页，不要拦路拦截'],
        ...
        dataTable(
            ...
            [
                ...
                ['7', '"你们能撑多久？"', '我们是广东链商平台运营方，母公司是母公司，有扎实的资本和技术背景。平台是长期战略，不是短期项目。', META.parent + '背书'],
                ['8', '"我朋友之前做类似的被坑了"', '很多平台确实有问题——收高额入驻费、承诺流量但不兑现、甚至搞多级分销。我们全部不一样——零固定费用+交易即服务费结算+单层推广，完全合规。', '差异化对比，强调合规'],
...
function buildB3_ConsumerScanGuide() {
    var children = [].concat(
        ...
        dataTable(
            ['场景', '标准话术', '配合动作'],
            [
                ['社区门口/电梯间', '您好！咱们社区的合作商家现在都在赋商通上——扫码注册有新人礼包！', '微笑展示小程序码卡片'],
                ['商铺门口（配合商家）', '欢迎光临！本店已入驻赋商通——扫码点单有优惠，新人还送¥25礼包！', '指向收银台旁的二维码立牌'],
        ...
        dataTable(
            ['问题', '标准回答'],
            [
                ['"这个安全吗？个人信息会泄露吗？"', '完全安全。我们只获取微信公开信息（昵称和头像），不收集身份证/银行卡等敏感信息。平台由广东链商平台运营方运营，母公司是母公司，正规企业。'],
...
function buildB4_PromoterRecruitment() {
    var children = [].concat(
        ...
        [
            '你对所在社区/片区非常熟悉，知道哪里有什么店', '你喜欢跟人聊天、交朋友，不排斥主动跟陌生人搭话', '你有一定的空闲时间（哪怕是周末或下班后）', '你对手机App/小程序操作比较熟练', '你有自己的人脉圈子（邻居群/家长群/同事群等）', '你希望有一份额外的灵活收入', '你能接受"收入不固定"——多劳多得，少劳少得', '你没有"一夜暴富"的期望——知道踏踏实实做才有回报', '你诚实守信，不会为了拉人而夸大宣传', '你对赋商通平台认可，愿意向别人推荐'
    ...
    var children = [].concat(
        ...
        dataTable(
            ...
            [
                ...
                ['P15', '竞品对比', '赋商通 vs 美团/抖音/有赞——五大维度全面对比', '2分钟'],
        ...
        calloutBox('讲述要点', [
            ...
            '④ 真实交易激励（交易即服务费结算）：零固定费用、零预充值——只在商家实际成交时收取服务费。'
        ], C.LIGHT),

        h3('P14 合规体系——"合规即壁垒"'),
        calloutBox('讲述要点', [
            '很多平台在合规上踩坑——资金池、多级分销、收益承诺——最后被监管清理。',
            '赋商通从第一天就建立三条红线：积分不兑现、不碰资金池、不承诺收益。',
        ...
        dataTable(
            ...
            [
                ...
                ['6', '平台能撑多久？', '我们的母公司是母公司，有足够的资本和战略定力。盈亏平衡仅需40,500笔/月，财务模型健康。'],
                ...
                ['11', '你们融过资吗？', '目前由母公司战略支持。根据业务发展需要适时启动外部融资。'],
    ...
    var children = [].concat(
        ...
        dataTable(
            ...
            [
                ...
                ['4', '怕平台做不起来', '"我们有母公司的战略支持，这不是短期项目"', META.parent + '背书+盈亏平衡仅40,500笔/月。'],
...
function buildD4_InvestorFAQ() {
    ...
    children = children.concat(faqBlock('一、平台定位与商业模式（8题）', [
        ['赋商通到底是做什么的？', '我们是面向社区商业的数字经营平台。帮助社区商家（餐饮/零售/服务）建立线上店铺、实现会员管理和复购提升。核心差异化在于"跨店通兑"——消费者在任意商家获得的代金券/积分/消费金，可以在全平台通用。'],
        ...
        ['平台的收入模式是什么？', '平台收入=商家交易额×服务费率。不靠广告、不靠排名竞价、不靠入驻费——纯交易服务费结算模式。这确保平台利益与商家利益一致——商家交易越多，平台收入越多。'],
        ...
        ['"交易即服务费结算"是什么意思？', '零固定费用、零预充值、零保证金——平台只在商家实际产生交易时收取服务费。商家没有交易=平台没有任何收入。这跟收年费/入驻费的模式有本质区别——我们不从商家"没赚到的钱"里收费。']
    ], C.MAIN));
    ...
    children = children.concat(faqBlock('二、财务与盈利（7题）', [
        ...
        ['后续需要融资吗？', '目前由母公司战略支持，可以覆盖从0到盈亏平衡的运营成本。后续根据扩张速度决定是否引入外部融资——如果城市复制速度远超预期，可能需要融资加速。但核心商业模式不依赖资本驱动。'],
    ...
    children = children.concat(faqBlock('四、市场竞争（7题）', [
        ['最大的竞争对手是谁？', '目前没有完全对标的直接竞争对手。美团本地生活是最强的间接竞争对手（但做的是到店+外卖，不是社区复购）。省团生活是较接近的竞品（异业联盟模式，62万+商家），但省团不做跨店通兑和交易即服务费结算。'],
        ...
        ['平台的护城河是什么？', '五道护城河：①跨店通兑的网络效应 ②交易即服务费结算的商家信任 ③合规体系的法律壁垒 ④3级管理体系的渠道深度 ⑤先发优势形成的品牌认知和数据积累。'],
    ...
    children = children.concat(faqBlock('六、团队与执行（5题）', [
        ['创始团队背景是什么？', '核心团队来自广东链商平台运营方（母公司子公司）。创始人及管理层具有电商平台运营、社区商业拓展、品牌策划的复合背景。技术团队有丰富的微信生态开发经验。'],
        ...
        ['如果核心人员离开怎么办？', '平台的核心竞争力不在于个别人员，而在于体系——合规框架、分账规则、品牌定位、技术架构——这些都是制度化而非人格化的。且母公司提供战略和人才储备支持。'],
        ['如何招募和培训推广者？', '推广者主要从两个渠道来：①活跃消费者的自然转化 ②社区意见领袖的定向招募。培训通过在线平台完成（30分钟视频+考核），服务站提供线下辅导。零门槛进入+灵活工作时间+交易即服务费结算=持续吸引推广者。']
    ], C.GRAY));
...
function buildD5_EventSigning() {
    var children = [].concat(
        ...
        spacer(200),
        p('甲乙双方经友好协商，就乙方加入赋商通平台（以下简称"平台"）开展合作事宜，达成以下意向：', { bold: true }),
        divider(),
...
// =============================================================================
async function main() {
    console.log('🚀 开始生成赋商通 · 市场拓展营销工具包 V1.0');
    console.log('══════════════════════════════════════════\n');
